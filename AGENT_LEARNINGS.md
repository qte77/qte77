...
### Dependabot groups need a dedicated `applies-to: security-updates` entry, or security fixes ship as unmergeable stragglers

- **Context**: Any repo using `dependabot.yml`'s `groups` key to bundle version updates for an ecosystem (the estate-standard pattern — one PR per ecosystem instead of one per package).
- **Problem**: A group's `applies-to` defaults to `version-updates` only (confirmed against GitHub's own Dependabot docs) — security-triggered updates are NOT covered by that same group unless a second, dedicated group sets `applies-to: security-updates`. Without it, each vulnerable package gets its own separate PR instead of one bundle. Worse: if the CI audit gate (`pip-audit`/`npm audit`/etc.) fails on ANY known vulnerability, each of those separate PRs shows a "failing" check even when merging all of them together would pass cleanly — nobody realizes the PRs are interdependent, and they sit stale. Observed directly: three such PRs (`qte77/web-recon-kit` — pip, httpx2, httpcore2) sat unmerged for 10 days for exactly this reason.
- **Solution**: Add a second group per ecosystem with `applies-to: security-updates` (same `patterns: ["*"]`, can't be combined into the version-updates group — GitHub requires separate entries per `applies-to` value). If several such PRs are already stuck: `@dependabot rebase` each one in turn as you merge them (a merge changes `main`, immediately putting the next one `BEHIND`), and expect the last one to sometimes report "no longer needed" if an earlier bump already pulled its fix in transitively — verify with a fresh audit run against the merged `main`, don't just trust Dependabot's own "up to date now" comment.
- **References**: `qte77/web-recon-kit#39` (the fix), `qte77/web-recon-kit#33`/`#34`/`#35` (the stragglers this fixed).
