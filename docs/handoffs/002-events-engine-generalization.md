...
Nothing built yet. This generalizes the **calendar/venue events** domain shared by **[redacted]**
(Python/Cloudflare, no-build vanilla JS) and **[redacted]** (React/Vite/Supabase, has the OSM map) into
...
React/Vite ([redacted] migrates off vanilla + relaxes its strict CSP). [redacted] carries the paired
ADRs — `docs/decisions/0007` (D1, PR #85, open) and `0008` (React, PR #86, open) — and a carry-over
`docs/handoffs/012-*`. **This plan + handoff were authored from the [redacted] session and are UNCOMMITTED
...
1. **Build `events-core` FIRST (phase 1).** Backend-only, no React, reaches green now, and it delivers
   [redacted]'s durability + the coordinates the map needs, shared with [redacted] immediately. Do NOT
   start the React migration first.
2. **`events-core`** = new `qte77/events-core`, Python, `uv`. Mirror polyfetch's "call from another repo
   via `uv run --directory ../events-core`" contract. Port the models from [redacted]
   `src/fo_scraper/models.py`; port geocode from [redacted]'s offline ops pipeline (OSM Nominatim); two
   store adapters behind one `EventStore` Protocol — **D1** ([redacted]) + **Supabase** ([redacted], from
   `useEventsQuery.ts` / `events_enriched`). TDD: models + dedupe (`sha1(source_id|norm(name)|start_date)`)
   + geocode tests red → green.
3. **Wire consumers to `events-core`.** [redacted] `build_ui_data.py` → `events_core.export.snapshot(D1Store())`
   (keep its `--require-events`/`--require-sitekey` guards + `sources.json`/`config.json` local).
   [redacted] swaps its inline model/geocode for `events-core` + the Supabase adapter.
4. **`events-ui` (phase 2)** — React/Vite, consumes `@qte77/ui-theme`; seed from [redacted] components
   (Map superset) + [redacted] (filters/Insights/Trip/export superset). Publish to GitHub Packages.
5. **Then** [redacted] React migration (phase 3, per its ADR-0008), shared map + Ctrl+K (phase 4),
...
- Create `qte77/events-core` (repo + `pyproject.toml`, `uv`); branch `feat/events-core-models`; port
  `Event`/`Source`/`Provenance` from [redacted] `src/fo_scraper/models.py` (drop FO-specific scoring;
...
- **[redacted] is at `/workspaces/[redacted]/[redacted]`** (different parent, NOT `/workspaces/qte77`), and is
  React/Vite/Supabase — do not assume [redacted]'s stack. [redacted]'s GitHub repo is `qte77/[redacted]`
  (double underscore).
- **Do NOT copy [redacted]'s map CSP** (`img-src https:` wildcard) or its disabled attribution. [redacted]
...
- [redacted] CSP is strict `default-src 'self'`; ADR-0008 relaxes it only to the minimum under Vite (keep
