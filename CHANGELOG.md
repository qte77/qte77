...
- `assets/images/icons/`: Tools-row marks for Model Context Protocol, uv (Astral), and Markdown — official CC0 mark paths from [Simple Icons](https://github.com/simple-icons/simple-icons) (like `claude.svg`). `uv.svg` ships in the official Astral brand purple (`#DE5FE9`) as a single colored mark; `mcp-*` and `markdown-*` (monochrome `#000000`) ship paired light/dark like the other monochrome icons. Wired into the README `## Tools` row; provenance noted in `assets/images/icons/README.md`
