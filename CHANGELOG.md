...
- `brand/ui-kit/tailwind/tokens.css` + `brand/ui-kit/package.json` + `.github/workflows/publish-ui-theme.yml`: publish `brand/ui-kit` as the **`@qte77/ui-theme`** npm package (GitHub Packages) so the Vite/React apps (agenthud-agui-a2ui, ldnmxx-hack) consume the EyeRest theme instead of hand-copying the Tailwind `@theme` block into `src/index.css`. `gen_ui_kit.py` now emits the token block as a Tailwind v4 `@theme` cascade (`--color-*` / `--font-*` / `--radius-*`) from the same `DESIGN.md` source; `--check` covers both generated files. Publish is idempotent (skips if the version is already up) and gated on `gen_ui_kit.py --check`. Executes qte77/agenthud-agui-a2ui#211 / plan 013 Wave 2 (shared-lib extraction)
- `brand/DESIGN.md`: **"Motion & effects"** policy section — the flat-surface mandate made explicit and machine-checkable (elevation from border+surface tone, never shadow; motion only if functional/subtle/`prefers-reduced-motion`-guarded; no gloss/gradient/pill). `@qte77/ui-theme` ships no shadow token and downstream packages lint against `box-shadow`/`*-gradient`/raw hex; closes the shadow-token upstream-promotion request tracked in #148
